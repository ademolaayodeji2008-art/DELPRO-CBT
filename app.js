...
async function fetchUserData(user) {
    try {
        ...
        else { currentUserData = { email: user.email, trialsLeft: 2, isSubscribed: false, createdAt: firebase.firestore.FieldValue.serverTimestamp() }; await db.collection('users').doc(user.uid).set(currentUserData); }
        authScreen.style.display = 'none'; examSelectionScreen.style.display = 'block'; document.getElementById('logged-out-nav').style.display = 'none'; document.getElementById('user-profile-badge').style.display = 'flex'; document.getElementById('user-name-display').innerText = currentUserData.email.split('@')[0]; updateTrialBadge();
...
async function startLiveApiExamFlow(exam, subject, btn) {
    ...
    try {
        const res = await fetch(`https://questions.aloc.com.ng/api/v2/m?subject=${getAlocSubjectCode(subject)}`, { headers: { 'AccessToken': 'ALOC-a8e00470020b18e5852d' } });
...
startDrillBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        for (let i = 0; i < selectedSubjects.length; i++) {
            ...
            const response = await fetch(`https://questions.aloc.com.ng/api/v2/m?subject=${getAlocSubjectCode(subjectName)}`, { headers: { 'AccessToken': 'ALOC-a8e00470020b18e5852d' } });
...
// ==========================================
// 7. CBT CANVAS & INSTANT FEEDBACK LOGIC
// ==========================================
...
function calculateScore() {
    clearInterval(timerInterval); let s = 0; quizData.forEach((q, i) => s += (userAnswers[i] === q.answer ? 1 : 0));
    document.querySelector('.cbt-main').style.display = "none"; document.querySelector('.cbt-footer').style.display = "none"; document.querySelector('.cbt-topbar').style.display = "none"; subjectTabsContainer.style.display = "none"; document.getElementById('result-area').style.display = "block"; headerNav.style.display = "flex"; document.getElementById('score-display').innerText = `${s}/${quizData.length}`;
}
